...
def get_LAcrime_geodata(la_map):
    """
    process the la crime data and return as geodata set,
    that we can use in all the part of project.
...
def get_LAmap():
    """
    The area name in la_map is a little different with crime data.
    Process la map data that make area name match with
    la crime data. Return after process data.
    We can join la map data to crime data after area name is consist.
...
def join_map_crime(la_crime_data, la_map):
    ...
    count of crime. And processed data for location graph
    """
    ...
    # but it took so long to run the dissolve funciton, here is the code:
    # crime_count = gpd.sjoin(la_map, la_crime_data, how='inner', op='intersects')
    # crime_count = crime_count[["APREC", "count", "geometry"]]
    # crime_count = crime_count.dissolve(by="APREC", aggfunc='sum')
...
def crime_type_data(la_crime_data):
    ...
    because it filter out premises that have low count.
    """
...
def wapon_data_process(la_crime_data):
    # limited data to top 10 wapon that invoded in crime
